@extends('layouts.main')

@section('content')
<div class="staticpages">
	<h3 class="section-title style2 text-center">
	<span>Corporates</span>
</h3>
<div class="staticpages">
	<span class="terms-text">Corporate gifting:</span>
	<br/>
	Funfest offers corporate gifting services to provide your company with gifting solutions which are not only innovative but also personalized.
	<br/>
	We deliver quality flowers with chocolates, cakes and gifts across India at highly affordable rates so wherever your client, customer or employee might be, we will deliver!
	<br/>
	Depending on the number departments/levels of management in the organization, a custom plan is created for each department/management level to not only gift employees on special occasions like birthdays, promotions, weddings, anniversaries etc but also for clients and customers of the company to make them feel truly cared for. A great gift can strengthen ties and create a memory which lingers on forever creating an everlasting impression.
	<br/>
	We offer seamless monthly/bi-weekly billing. For us customer satisfaction is paramount because we ensure that no stone is left unturned in meeting your expectations.
	<br/>
	By choosing Funfest.com, you are guaranteed professionalism, speedy delivery, quality arrangements and great service.
	<div class="center">
		<img src="/img/franchise/BT.jpg">
		<img src="/img/franchise/PP.jpg">
	</div>
	
	<span class="terms-text">Hotels:</span><br/> We have been supplying quality flowers to hotels for over 15 years. 
	Our main focus is quality and timely delivery of flowers. 
	We have a dedicated team for making floral arrangements in 5 star hotels.
	<br/>
	For further enquiries, please email flowers.jpagro@gmail.com
	<br/>
</div>
@stop